...
export default function Page() {
  ...
  const getAllResult = async () => {
    ...
    const updatedResultsPromises = currentProgram.resultList.map(async (item) => {
      if (item.type === 'image') {
        try {
          const res = await getImageResult({ path: item.path });
      ...
      if (item.type === 'text') {
        try {
          const res = await getTextResult({ path: item.path });
...
//   const getAllResult = () => {
//     const temp = { ...programList };
//     const results = temp[selectedProgram].resultList;
//     results.forEach((item,index) => {
//         if (item.type === 'image') {
//             getImageResult({ path: item.path }).then(res => {
//                 const imageBlob = new Blob([res.data], { type: res.data.type });
//                 const imageUrl = URL.createObjectURL(imageBlob);
...
//         } 
//         if (item.type === 'text') {
//             getTextResult({ path: item.path }).then(res => {
